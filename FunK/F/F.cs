...
namespace FunK
{
    public static partial class F
    {
        ...
        public static Maybe<T> Head<T>(IEnumerable<T> list)
          => list.Any() ? Just(list.First()) : Nothing;

        private static Try<object> getProperty(string path, object instance) => () =>
        {
            var pp = path.Split('.');
        ...
        public static Exceptional<object> Get(string path, object instance) =>
          getProperty(path, instance).Run();

        public static T GetOr<T>(T defaultValue, string path, object instance) =>
          getProperty(path, instance).Run().Match((ex) => defaultValue, (t) => (T)t);
