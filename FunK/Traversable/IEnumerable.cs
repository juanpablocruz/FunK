...
namespace FunK
{
  ...
  public static class IEnumerableTraversable
  {
    ...
    static Maybe<IEnumerable<R>> TraverseARec<T, R>(this IEnumerable<T> list
         , Func<T, Maybe<R>> func)
         => list.Match(
            Empty: () => Just(Enumerable.Empty<R>()),
            Otherwise: (t, ts) => Just(Cons<R>())
               .Apply(func(t))
               .Apply(ts.TraverseARec(func)));

    // applicative traverse
    public static Maybe<IEnumerable<R>> TraverseA<T, R>(this IEnumerable<T> list
       , Func<T, Maybe<R>> f)
       => list.Aggregate(
          seed: Just(Enumerable.Empty<R>()),
          func: (optRs, t) => Just(Append<R>())
                                 .Apply(optRs)
                                 .Apply(f(t)));

    // monadic traverse, recursive (reference only)
    static Maybe<IEnumerable<R>> TraverseMRec<T, R>(this IEnumerable<T> list
       , Func<T, Maybe<R>> func)
       => list.Match(
          Empty: () => Just(Enumerable.Empty<R>()),
    ...
    // monadic traverse
    public static Maybe<IEnumerable<R>> TraverseM<T, R>(this IEnumerable<T> ts, Func<T, Maybe<R>> func) => Traverse(ts, func);

    public static Maybe<IEnumerable<R>> Traverse<T, R>
       (this IEnumerable<T> ts, Func<T, Maybe<R>> func)
       => ts.Aggregate(
          seed: Just(Enumerable.Empty<R>()),
          // Maybe<[R]> -> T -> Maybe<[R]>
          func: (optRs, t) => from rs in optRs
